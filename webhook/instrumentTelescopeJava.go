...
func javaTelescopeInstrumentation(pod corev1.Pod, instrRule *InstrumentationRule) []patchOperation {
	...
	patchOps = append(patchOps, addTelescopeJavaAgentVolume(pod, instrRule)...)
...
func addTelescopeJavaEnvVar(pod corev1.Pod, instrRules *InstrumentationRule, containerIdx int) []patchOperation {
	patchOps := []patchOperation{}
	...
	patchOps = append(patchOps, patchOperation{
		...
		Value: corev1.EnvVar{
			...
			Value: getTelescopeJavaOptions(pod, instrRules),
		},
	})
...
func getTelescopeJavaOptions(pod corev1.Pod, instrRules *InstrumentationRule) string {
	javaOpts := " "
	...
	javaOpts += "-javaagent:/opt/telescope-agent/cisco-telescope-javaagent.jar "
	javaOpts += instrRules.InjectionRules.JavaCustomConfig + " "

	return javaOpts
}

func addTelescopeJavaAgentVolumeMount(pod corev1.Pod, instrRules *InstrumentationRule, containerIdx int) []patchOperation {
	patchOps := []patchOperation{}
	patchOps = append(patchOps, patchOperation{
		Op:   "add",
		...
		Value: corev1.VolumeMount{
			...
			Name:      "telescope-agent-repo-java", //TODO
		},
	})
	return patchOps
}

func addTelescopeJavaAgentVolume(pod corev1.Pod, instrRules *InstrumentationRule) []patchOperation {
	patchOps := []patchOperation{}
	patchOps = append(patchOps, patchOperation{
		Op:   "add",
		Path: "/spec/volumes/-",
...
func addTelescopeJavaAgentInitContainer(pod corev1.Pod, instrRules *InstrumentationRule) []patchOperation {
	patchOps := []patchOperation{}
	...
	patchOps = append(patchOps, patchOperation{
		...
		Value: corev1.Container{
			Name:            "telescope-agent-attach-java", //TODO
